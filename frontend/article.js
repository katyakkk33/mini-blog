...
async function loadArticle() {
  const res = await (window.Logger?.apiFetch(`${API_BASE}/api/articles/${articleId}`) ?? fetch(`${API_BASE}/api/articles/${articleId}`));
...
async function loadComments() {
  ...
  const res = await (window.Logger?.apiFetch(`${API_BASE}/api/articles/${articleId}/comments`) ?? fetch(`${API_BASE}/api/articles/${articleId}/comments`));
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
...
els.cForm.addEventListener("submit", async (e) => {
  ...
  try {
    const res = await (window.Logger?.apiFetch(`${API_BASE}/api/articles/${articleId}/comments`, {
      method: "POST",
...
els.comments.addEventListener("submit", async (e) => {
  ...
  try {
    const res = await (window.Logger?.apiFetch(`${API_BASE}/api/comments/${commentId}/replies`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ author, content }),
    }) ?? fetch(`${API_BASE}/api/comments/${commentId}/replies`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ author, content }),
    }));
